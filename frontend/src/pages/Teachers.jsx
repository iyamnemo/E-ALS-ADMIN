...
const filteredTeachers = useMemo(() => {
  ...
  return teachers.filter(teacher =>
    teacher.first_name.toLowerCase().includes(lowerCaseSearchTerm) ||
    teacher.middle_name.toLowerCase().includes(lowerCaseSearchTerm) ||
    teacher.last_name.toLowerCase().includes(lowerCaseSearchTerm) ||
    teacher.email.toLowerCase().includes(lowerCaseSearchTerm) ||
    `${teacher.first_name.toLowerCase()} ${teacher.middle_name.toLowerCase()} ${teacher.last_name.toLowerCase()}`.includes(lowerCaseSearchTerm) ||
    `${teacher.first_name.toLowerCase()} ${teacher.last_name.toLowerCase()}`.includes(lowerCaseSearchTerm)
  );
}, [teachers, searchTerm]);
// --- End Search Logic ---
  ...
  const handleConfirmationSubmit = async () => {
    ...
    const url = editing
      ? `http://localhost:3001/api/teachers/${editingTeacherId}` 
  ...
  const handleDelete = async () => {
    ...
    try {
        const response = await fetch(`http://localhost:3001/api/teachers/${teacherToDelete.id}`, {
          method: 'DELETE',
        });
